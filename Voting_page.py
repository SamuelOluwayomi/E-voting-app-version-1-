...
canvas.create_text(460, 230, text="Enter Password To Confirm Vote", font=(('helvetica'), 15, ('bold')))
...
class cast_votes():
    ...
    def voting_system(self):
        if candidates_Box["values"] == " " and entry_password.get() != " ":
            messagebox.showerror("Error",  "All fields are required", parent=voting)
        else:
            messagebox.showinfo("Info", "you have successfully cast your vote")
...
entry_password.place(x=300, y=250)
check_pass.place(x=600, y=250)
...
show_password.place(x=600, y=280)
